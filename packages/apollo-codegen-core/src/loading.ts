...
import { buildClientSchema, Source, concatAST, parse, DocumentNode, GraphQLSchema } from 'graphql';
...
export function loadInterpolatedQueries(inputPaths: string[], tagName: string = 'gql'): string[] {
  return inputPaths
    .flatMap(i => withGlobalFS(() => fg.sync(i, { cwd: process.cwd(), absolute: true })))
    .flatMap(inputPath => {
        ...
        literal.expressions.forEach((exp: any) => {
          ...
          switch (parentType) {
            case 'VariableDeclarator':
              ...
              variableToValue.set(exp.name, parent.init.quasi.quasis[0].value.cooked);
              break;
            default:
  ...
  mapping.forEach((file, exportedValue) => {
    try {
      ['.js', '.jsx', '.ts', '.tsx', '.mjs'].forEach(ext => {
        const filename = (file.startsWith('/') ? process.cwd() : options.fileRoot || '') + file + ext;
        ...
        astTypes.visit(ast, {
          visitProgram: (path: any) => {
            const value = path.scope.getBindings()[exportedValue][0].parent.value.init.quasi.quasis[0].value
              .cooked;
      ...
      if (!variableToValue.has(exportedValue)) {
        throw new Error('Unable to locate import ' + file + ' with ' + options.fileRoot);
      }
    } catch (e) {
      console.log(e);
    }
